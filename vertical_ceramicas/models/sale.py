...
class sale_order_line(models.Model):
    ...
    def onchange_product(self):
        " " "
        Ponemos un dominio al campo route_id para que traiga solo las rutas a
        los almacenes donde hay producto
        " " "
        for line in self:
    ...
    def product_id_change(
        self, ppp, pricelist, product, qty=0, uom=False, qty_uos=0, uos=False,
        name='', partner_id=False, lang=False, update_tax=True,
        date_order=False, packaging=False, fiscal_position=False, flag=False):
        """ Hereda de la funcion product_id_change que es llamada cuando cambia
            el producto o la cantidad, se intercepta el res y se le agrega m2
            en el name
        """
        ...
        if product:
            ...
            if prod.prod_in_box != 0:
                # agregamos los metros cuadrados, al value/name. A veces el
                # super decide que no tiene que modificar el name entonces no
                # está en el dict, en ese caso lo agrego y le pongo el
                # name_get + los metros cuadrados
                try:
    ...
    def _check_discount(self):
        ...
        """
        manager = self.env['res.users'].has_group(
            'vertical_ceramicas.group_reves_manager_users')
