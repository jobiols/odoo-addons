...
class sale_order_line(osv.osv):
    ...
    def product_id_change_with_wh(self, cr, uid, ids, pricelist, product,
        ...
        warning = {}
        # UoM False due to hack which makes sure uom changes price, ...
        ...
        warning_msgs = res_packing.get('warning') and res_packing['warning'][
            'message'] or ''

        if product_obj.type == 'product':
            # determine if the product needs further check for
            # stock availibility
            is_available = self._check_routing(cr, uid, ids, product_obj,
            ...
            if not is_available:
                ...
                # verificar donde está el producto y si está en otro local
                # sacamos otro warning
                locations = self.calc_formated_stock(cr, uid, ids, product_obj,
                                                     context)
                if locations:
