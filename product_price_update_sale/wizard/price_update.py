...
class ProductPriceUpdate(models.TransientModel):
    _name = 'product.price.update.so'

    name = fields.Many2one('product.category', 'Categoría', required=True)
    ...
    def change_products(self):
        domain = []
        if self.name:
            domain = domain + [('categ_id', '=', self.name.id)]
    ...
    def confirm(self):
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'product.price.update.so',
            'target':'new',
            'context':{
