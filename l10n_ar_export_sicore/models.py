...
class AccountExportSicore(models.Model):
    ...
    def _compute_files(self):
        ...
        filename = 'Sicore_%s_%s.txt' % (str(self.date_from),
                                         str(self.date_to))
        self.export_sicore_filename = filename
        # sin el if revienta mal
        if self.export_sicore_data:
            self.export_sicore_file = base64.encodestring(
                self.export_sicore_data.encode('ISO-8859-1'))
    ...
    def compute_sicore_data(self):
        ...
        for payment in payments:
            ...
            string = string + payment.payment_date[
                              8:10] + '/' + payment.payment_date[
                                            5:7] + '/' + payment.payment_date[
                                                         :4]
            # 3er campo nro de comprobante - imprimimos nro de
            # retenciontring = string + payment_data['withholding_number'].zfill(16)
            string = string + '    ' + payment.payment_group_id.name[
                                       10:].zfill(12)
            # 4to campo amoun
            if payment.payment_group_id.to_pay_amount > 0:
            ...
            string = string + str(
                payment.tax_withholding_id.tax_group_id.afip_code).zfill(3)[:3]
            ...
            string = string + payment.partner_id.main_id_number + '         '
